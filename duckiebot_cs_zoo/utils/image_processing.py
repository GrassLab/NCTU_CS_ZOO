...
def color_balance(img, percent=70, image_height_clip_ratio=0.3,
                  clip_low=True, clip_high=True):  # "Illumination compensation" in the duckietown paper
    """
...
def calc_normal_(in_range_map, lines, extend_l):
    ...
    normal = np.hstack([-t[:, [1]], t[:, [0]]])  # => type1 (t CCW 90 degree)
    ...
    normal[inside_color_2[valid_lines] == 255] *= -1  # => type2 (t CW 90 degree)
    # Normal type (1 or 2)
...
def calc_normal(in_range_map, lines, extend_l=5, vis=False):
    # Make sure a normal vector is heading toward its corresponding color chunk
    if not vis:
            ...
            if is_changed:
                ...
                for l, n in zip(lines_new, normal):
                    ...
                    cv2.arrowedLine(vis_map, tuple(mid),
                                    ...
                                    line_type=8,
                                    tipLength=0.5)
                    cv2.line(vis_map, tuple(l[0:2] * UP_SCALE), tuple(l[2:] * UP_SCALE), rand_color, 2)
                is_changed = False
