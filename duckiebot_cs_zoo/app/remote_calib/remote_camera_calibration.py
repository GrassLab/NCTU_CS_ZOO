...
def camera_calibration_chessboard(square_sz, board_size, folder_path, intrinsics_name="intrinsics.pkl"):
    """
...
def validate_calibration(camera, square_sz, board_size, folder_path, intrinsics_name="intrinsics.pkl", alpha=0,
...
def run_cam_calib():  # Camera calibration
    client = DuckiebotClient()
    folder_path = osp.join(osp.dirname(osp.dirname(osp.abspath(__file__))), 'data', 'camera_calibration')
    if camera_calibration_chessboard(0.031, (8, 6), folder_path):
        validate_calibration(client.camera, 0.031, (8, 6), folder_path, alpha=0,
                             draw_chessboard=False)  # compare alpha=0/1
