...
def lane_detection_pipeline_single():
    ...
    COLOR_RANGE_FILE = 'my_colors_challenge11.pkl'  # TODO: decide your filename
    COLOR_BALANCE_FILE = 'my_balance_challenge11.pkl'  # TODO: decide your filename
    """
    Step1: Color Balance Pipeline
    """
    print(
        'Place the duckiebot 1. in front of the cyan line 2. in front of the green line (face to the right side of the map, for viewing RED and YELLOW closer) , press r to save, q to leave')
    view_imgs = []
    while True:
        rect_img = duckiebot.get_rectified_image()
        k = show_img(rect_img, "Duckiebot", 30, destroy=False)
        if k == 'r':
            print(f'save {len(view_imgs) + 1} image')
            view_imgs.append(rect_img)
        elif k == 'q' and len(view_imgs) > 0:
            break
    rect_img_comb = np.hstack(view_imgs)
    ...
    img = b3  # TODO: decide your parameters (b1 or b2 or b3 and keep their clip_low/clip_high settings in the next line)
    luts = improc.calculate_color_balance(rect_img, percent=percent, clip_low=True,
                                          clip_high=True)  # TODO: Decide your parameters, same as the choice for img,the color balance parameters will be saved
    lane_dt.set_color_balance(luts, COLOR_BALANCE_FILE)

    img_comb = lane_dt.apply_color_balance(rect_img_comb)  # apply consistent color balance

    """
    Step2: Color Segmentation Using HSV (Red "may" contains 2 ranges)
    """
    lane_dt.tune_color_ranges(img_comb, file_name=COLOR_RANGE_FILE)  # Use this at the first time
