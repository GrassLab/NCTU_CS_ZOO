...
def camera_calibration_chessboard(duckiebot, square_sz, board_size, file_path):
    """
    Please take around 20-30 images from varies angles, positions(left-right , far-near)

    :param duckiebot: DuckiebotClient object
    ...
    h, w = duckiebot.get_stream_img().shape[:2]
    # Chessboard Mapping
...
def validate_calibration(duckiebot, square_sz, board_size, file_path, alpha=0, draw_chessboard=False):
    """
...
def run_cam_calib():  # Camera calibration
    ...
    camera_calibration_chessboard(duckiebot, 0.031, (8, 6), file_path)
    validate_calibration(duckiebot, 0.031, (8, 6), file_path, alpha=0, draw_chessboard=True) # compare alpha=0/1

    # Write calibration data to the duckiebot client
    rms, camera_matrix, dist_coeffs = joblib.load(file_path)
    # Uncomment to save the result
    # duckiebot.set_cam_calibration(camera_matrix, dist_coeffs, 480, 640)


# Check rectified images
def check_rectified():
    duckiebot = DuckiebotClient('duckiebotchai.local', 54761, restore_data=True)
    while True:
        img = duckiebot.get_stream_img()
        rect = duckiebot.get_rectified_image(img)
